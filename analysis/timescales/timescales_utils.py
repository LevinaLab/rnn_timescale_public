...
def comp_acs(device, data_path, save_path, curriculum_type, task, network_number, N_max_range, T, num_neurons,
             num_trials, max_lag, fit_lag, burn_T, strict=False, mod_model=False, mod_afunc=torch.nn.LeakyReLU,
             affixes=[]):
    ...
    for i, N in enumerate(N_max_range):
        ...
        # loading the model
        print('N = ', N)
        rnn = load_model(curriculum_type=curriculum_type, task=task, network_number=network_number, N_max=N,
                         N_min=N_min, device=device, base_path=data_path, strict=strict, mod_model=mod_model,
                         mod_afunc=mod_afunc, affixes=affixes)
