...
df_all = pd.read_pickle('../../results/df_ablate_tau_mod.pkl')
...
for i_N, N in enumerate([5, 30]):
    for i_network_type, network_type in enumerate(network_types):
        # ax = axs[i_N, i_network_type]
        ax = axs[i_N * 2 + i_network_type]
        df = df_all.loc[network_type, network_numbers, N]
        if len(df) == 0:
            continue
